...
const QRSparseT = SparseArrays.SPQR.QRSparse
...
@testset "eltypes: Float64/ComplexF64 succeed, others error (not silent Float64)" begin
    for T in (Float64, ComplexF64)
        A = rand_sparsedense(50, 3; seed = 4, T = T)
        b = T <: Complex ? (randn(50) .+ im .* randn(50)) .|> T : randn(T, 50)
        F = qr(A)
        @test F isa SparseWithDenseRowColQRAugmented{T}
        @test relerr(F \ b, exact_solve(A, b)) < 1.0e-11
    end
    # Float32/ComplexF32 are intentionally unsupported (SuiteSparseQR single precision is
    # version-dependent — it throws on Julia 1.11); they must error, not silently upcast.
    for T in (Float32, ComplexF32, Rational{Int}, Int, BigFloat)
        # build an exact-typed matrix (randn→Int would itself throw before qr is reached)
        nn = 20
        S = spdiagm(0 => fill(T(4), nn), 1 => fill(T(-1), nn - 1), -1 => fill(T(-1), nn - 1))
        U = zeros(T, nn, 2); U[1, 1] = one(T); U[2, 2] = one(T)
        V = zeros(T, 2, nn); V[1, 1] = one(T); V[2, 2] = one(T)
        A = SparseWithDenseRowColMatrix(S, U, V)
        @test_throws ArgumentError qr(A)   # must error, not return a Float64 factorization
    end
end
...
@testset "adjoint & transpose solve (lazy cached QaugH)" begin
    ...
    @test F.QaugH isa QRSparseT                    # lazily built
    cached = F.QaugH
    @test relerr(F' \ b, M' \ b) < 1.0e-11
...
@testset "ill-conditioned ASSEMBLED A does not spuriously throw (SPQR deflation off)" begin
    # Regression: SuiteSparseQR's default `tol = -2` deflates ill-conditioned-but-nonsingular
    # columns and would make qr(A) throw SingularException on matrices that factorize/klu solve
    # fine. With deflation disabled, an assembled A up to cond ~1e13 must factor and solve to
    # roughly its conditioning-limited accuracy (κ(A)·eps), matching the LU path's behavior.
